...
with h5py.File(train_file, 'r') as file:
    ...
    train_nonlinear_64_sparse_interp = torch.tensor(file['train_nonlinear_64_sparse_interp'][:], device=device)

train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(train_nonlinear_64,
                                                                          train_vorticity_64,
                                                                          train_nonlinear_64_sparse_interp),
...
torch.save(model.state_dict(), 'SparseNonlinearModel_Interp.pth')
