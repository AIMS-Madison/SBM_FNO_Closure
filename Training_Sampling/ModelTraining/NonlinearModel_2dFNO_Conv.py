...
with h5py.File(train_file, 'r') as file:
    ...
    train_nonlinear_64_sparse_normalized = torch.tensor(file['train_nonlinear_64_sparse_normalized'][:], device=device)

train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(train_nonlinear_64,
                                                                          train_vorticity_64,
                                                                          train_nonlinear_64_sparse_normalized),
...
torch.save(model.state_dict(), '../Trained_Models/SparseNonlinearModel_Conv.pth')